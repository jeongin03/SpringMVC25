...
@Controller // servlet-context.xml에서 <context:component-scan base-package="org.zerock.controller" />
...
public class SampleController {
	...
	public String ex01(SampleDTO dto) { // SamleDTO dto = new SampleDTO();
		
		log.info("SampleController.basicGet2() 메서드 실행");
		log.info("get방식 반응");	
		log.info("입력 dto 확인 : " + dto);
		
		return "ex01" ; // WEB-INF/views/ex01.jsp -> servlet-context.xml 이 하는 일
	} // ex01 메서드 종료
	...
	private String ex02(@RequestParam("owner") String name, @RequestParam("age") int age) { 
		                // 프론트 변수와 백엔드의 변수가 다를때 !! (정인이는 일치시키셈)
		log.info("name : " + name);
		log.info("age : " + age);	
	...
	public void ex05() {
	    ...
	    // 리턴이 void인 경우에는 자동으로 [/WEB-INR/views/ sample/ex05.jsp] 를 찾는다.
	    // 초보자 사용금지
	}
